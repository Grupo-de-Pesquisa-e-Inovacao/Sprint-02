...
SELECT tc.medicao as Temperatura,
	date_format(tc.dt_hora, '%d/%m/%Y - %H:%i') as 'Data & Hora',
    sensor.modelo as 'Modelo do sensor',
    sensor.num_serie as 'Número de série',
    CONCAT(aviario.nome, ' - ', setor.nome) as 'Aviário & Setor',
    CASE
		WHEN tc.medicao < 19 THEN 'Abaixo do ideal'
        WHEN tc.medicao > 28 THEN 'Acima do ideal'
        ELSE 'Temperatura ideal'
	END as Situação
    FROM temperatura_celsius tc
    JOIN sensor
		ON tc.fk_sensor = sensor.id
	JOIN setor
		ON sensor.fk_setor = setor.id
	JOIN aviario
		ON setor.fk_aviario = aviario.id order by tc.dt_hora asc;
        
        -- Temperatura agrupada por cada aviário
CREATE VIEW dadosAviario 
AS
SELECT 
	a.nome AviarioNome,
    s.nome SetorNome,
    sen.modelo ModeloSensor,
    tc.medicao Temperatura
FROM 
	aviario a
JOIN 
	setor s
